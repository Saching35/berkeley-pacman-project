...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        for act in allActions:
          tempVal.append(self.getQValue(state, act))
        if len(tempVal)>0:
          return max(tempVal)
        return bestVal
    ...
    def computeActionFromQValues(self, state):
        ...
        for act in allActions:
          tempVal = self.getQValue(state,act)
          if bestVal == tempVal:
            bestAct.append(act)
          
        bestChoice = random.choice(bestAct)
        return bestChoice
    ...
    def update(self, state, action, nextState, reward):
        ...
        self.qValues[(state, action)] = (1 - self.alpha) * oldValue + self.alpha * newValue
